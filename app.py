...
from forms import IdeaForm, CommentForm, ModeratorLoginForm, EditIdeaForm
...
def idea_detail(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # Автоматическое изменение статуса при отклонении через комментарий
            if form.is_public.data and any(word in form.text.data.lower() 
                                         for word in ['отклонен', 'отклонено', 'отказано']):
                idea.status = 'Отклонено'
                flash('Идея автоматически помечена как "Отклонено"', 'warning')
    ...
    comments_query = db.select(Comment).where(Comment.idea_id == id)
    
    if not current_moderator:
        # Для обычных пользователей показываем только публичные комментарии
        comments_query = comments_query.where(Comment.is_public == True)
    
    comments = db.session.execute(
        comments_query.order_by(Comment.created_at.desc())
    ).scalars().all()
...
def edit_idea(id):
    ...
    if form.validate_on_submit():
        form.populate_obj(idea)  # Заполняем объект идеи данными из формы
        db.session.commit()
